...
class TestSettingsDefaults:
    ...
    def test_file_upload_defaults(self):
        s = Settings(_env_file=None)
        # In dev mode (non-frozen), _resolve_dir returns the relative string
        assert s.upload_dir == "uploads"
        assert s.max_upload_size_mb == 50

    def test_checkpoint_db_default(self):
        s = Settings(_env_file=None)
        # In dev mode (non-frozen), _resolve_file returns the relative string
        assert s.checkpoint_db_path == "hunt_sessions.db"
