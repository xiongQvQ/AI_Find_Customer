...
面向客户的 Windows 交付目标是：**双击安装包即可使用**。

用户侧步骤：

1. 双击 `AIHunter_...-setup.exe`
2. 按安装向导完成安装
3. 从开始菜单/桌面启动 AI Hunter

无需用户自行安装 Python、Node.js、Rust，也不需要命令行。

说明：`setup.exe` 是 NSIS 压缩安装包，体积（例如 80~120MB）通常会小于最终安装目录大小，这是正常现象。
